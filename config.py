...
# LOSS
_C.TRAIN.LOSS.ALPHA_TIME = 1.0  # 시간 도메인 MSE ↑
_C.TRAIN.LOSS.ALPHA_FREQ = 1.0  # 주파수 도메인 손실 ↓
_C.TRAIN.LOSS.ALPHA_CORR = 2.0  # 피어슨 상관 손실 ↑
_C.TRAIN.LOSS.ALPHA_DENORM = 8.0  # 진폭 복원용 MSE 대폭 ↑
_C.TRAIN.LOSS.ALPHA_WAVE_CORR = 1.0  # 파형-level Pearson loss (추가)
_C.TRAIN.LOSS.SIGMA = 1.0  # 주파수 타깃 분포 σ 축소
_C.TRAIN.LOSS.LOW_CUT = 0.75
_C.TRAIN.LOSS.HIGH_CUT = 3.00

# Crop / RoI / Tracking
_C.CROP = CN()
_C.CROP.DO_DYNAMIC_DETECTION = False
_C.CROP.DYNAMIC_DETECTION_FREQUENCY = 30
_C.CROP.USE_HYBRID = True            # False: Dynamic, True: Hybrid(CSRT tracker)
_C.CROP.USE_MEDIAN_FACE_BOX = False    # median/static fallback
_C.CROP.WIDTH = 128
_C.CROP.HEIGHT = 128
...
_C.TRAIN.DATA.PREPROCESS.DATA_TYPE = ["Standardized"]
...
_C.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH = 300  # 180
...
_C.TRAIN.DATA.PREPROCESS.CROP_FACE.DETECTION = _C.CROP.clone()
...
_C.TRAIN.DATA.PREPROCESS.BANDPASS.ORDER = 4  # Butterworth 필터 차수 예시
...
_C.VALID.DATA.PREPROCESS.DATA_TYPE = ["Standardized"]
...
_C.VALID.DATA.PREPROCESS.CHUNK_LENGTH = 300  # 180
...
_C.VALID.DATA.PREPROCESS.CROP_FACE.DETECTION = _C.CROP.clone()
...
_C.VALID.DATA.PREPROCESS.BANDPASS.ORDER = 4
...
_C.TEST.DATA.PREPROCESS.DATA_TYPE = ["Standardized"]
...
_C.TEST.DATA.PREPROCESS.CHUNK_LENGTH = 300  # 180
...
_C.TEST.DATA.PREPROCESS.CROP_FACE.DETECTION = _C.CROP.clone()
...
_C.TEST.DATA.PREPROCESS.BANDPASS.ORDER = 4
...
_C.UNSUPERVISED.DATA.PREPROCESS.DATA_TYPE = ["Standardized"]
...
_C.UNSUPERVISED.DATA.PREPROCESS.CHUNK_LENGTH = 300  # 160
_C.UNSUPERVISED.DATA.PREPROCESS.CROP_FACE = CN()
_C.UNSUPERVISED.DATA.PREPROCESS.CROP_FACE.DO_CROP_FACE = True
_C.UNSUPERVISED.DATA.PREPROCESS.CROP_FACE.DETECTION = _C.CROP.clone()
...
_C.UNSUPERVISED.DATA.PREPROCESS.CROP_FACE.USE_ADAPTIVE_ROI = False
...
_C.INFERENCE.EVALUATION_WINDOW.WINDOW_SIZE = 60  # 10
_C.INFERENCE.EVALUATION_WINDOW.WINDOW_STEP_SIZE = 7.5
...
def update_config(config, args):
    ...
    # ── CROP 설정 통합
    # 최상위 _C.CROP 을 각 영역의 DETECTION / INFERENCE.CROP 에 복제하여 전처리·추론 모두 동일 파라미터를 참조하게 함
    config.TRAIN.DATA.PREPROCESS.CROP_FACE.DETECTION = config.CROP.clone()
    config.VALID.DATA.PREPROCESS.CROP_FACE.DETECTION = config.CROP.clone()
    config.UNSUPERVISED.DATA.PREPROCESS.CROP_FACE.DETECTION = config.CROP.clone()
    config.INFERENCE.CROP = config.CROP.clone()
    ...
    # Unify bandpass cutoff: 모든 split 의 PREPROCESS.BANDPASS → TRAIN.LOSS 설정으로 덮어쓰기
    for split in ['TRAIN', 'VALID', 'TEST', 'UNSUPERVISED']:
        pre = getattr(config, split).DATA.PREPROCESS
        # BANDPASS가 정의된 split만 덮어쓰기
        if hasattr(pre, 'BANDPASS'):
            bp = pre.BANDPASS
            bp.LOW_CUT = config.TRAIN.LOSS.LOW_CUT
            bp.HIGH_CUT = config.TRAIN.LOSS.HIGH_CUT

    # ——— LABEL_TYPE 일관성 체크 ———
    lt = config.TRAIN.DATA.PREPROCESS.LABEL_TYPE
    if config.TOOLBOX_MODE in ['train_and_test', 'only_test']:
        assert lt == config.VALID.DATA.PREPROCESS.LABEL_TYPE == config.TEST.DATA.PREPROCESS.LABEL_TYPE, \
            f"Train/Valid/Test LABEL_TYPE must match, got {lt} / " \
            f"{config.VALID.DATA.PREPROCESS.LABEL_TYPE} / {config.TEST.DATA.PREPROCESS.LABEL_TYPE}"
    ...
    if config.UNSUPERVISED.DATA.EXP_DATA_NAME == "":
        config.UNSUPERVISED.DATA.EXP_DATA_NAME = "_".join([
            ...
            "LabelType{0}".format("_".join(config.UNSUPERVISED.DATA.PREPROCESS.LABEL_TYPE)),
            "Crop_face{0}".format(config.UNSUPERVISED.DATA.PREPROCESS.CROP_FACE.DO_CROP_FACE),
            "Dynamic_Det{0}".format(config.UNSUPERVISED.DATA.PREPROCESS.CROP_FACE.DETECTION.DO_DYNAMIC_DETECTION),
    ...
    config.UNSUPERVISED.DATA.CACHED_PATH = os.path.join(config.UNSUPERVISED.DATA.CACHED_PATH,
    ...
    if not ext:  # no file extension
        ...
        config.UNSUPERVISED.DATA.FILE_LIST_PATH = os.path.join(
            config.UNSUPERVISED.DATA.FILE_LIST_PATH,
