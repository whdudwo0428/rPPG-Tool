...
def unsupervised_predict(config, data_loader, method_name):
    ...
    diff_flag = int(getattr(config.UNSUPERVISED.DATA.PREPROCESS, "LABEL_TYPE", "") == "DiffNormalized")
    win_s  = config.INFERENCE.EVALUATION_WINDOW.WINDOW_SIZE
    step_s = config.INFERENCE.EVALUATION_WINDOW.WINDOW_STEP_SIZE
    ...
    for batch in tqdm(data_loader["unsupervised"], ncols=80):
        frames_batch = batch[0].cpu().numpy()  # (B, T, H, W, 3)
