...
class Frequencydomain_FFN(nn.Module):
    ...
    def forward(self, x):
        ...
        try:
            # ── FP16(half)에서는 cuFFT가 일부 크기를 지원하지 않으므로, FP16일 땐 FP32로 변환 후 FFT 실행
            if x.dtype == torch.float16:
                # FP16 → FP32로 변환
                x_fre = torch.fft.fft(x.to(torch.float32), dim=1, norm='ortho')
                # FFT 결과는 complex64 (FP32 실수+허수), 이를 다시 원래 디바이스로 복귀
                x_fre = x_fre.to(device).to(torch.complex64)
            else:
            ...
            # ── IFFT도 FP16일 땐 FP32 결과를 복구 형태로 사용
            if x_fre.dtype == torch.complex64:
                x = torch.fft.ifft(x_fre, dim=1, norm='ortho').real.to(torch.float32).to(device)
            else:
                x = torch.fft.ifft(x_fre, dim=1, norm='ortho').real.to(torch.float32).to(device)
        except RuntimeError:
            # ── GPU FFT가 실패(주로 FP16+비전형 크기)하면, CPU로 옮겨 FP32 FFT 수행
            if x.dtype == torch.float16:
                # FP16 → CPU & FP32
...
class PhysMamba(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # 마지막에 진폭 복원을 위한 learnable scale & bias
    ...
    def forward(self, x):
        ...
        # ── amplitude 복원을 위한 learnable scale & bias 적용 ──
        rPPG = self.ConvBlockLast(rPPG)  # (B, 1, D)
        # (B,1,D) 형태인 rPPG에 output_scale, output_bias 브로드캐스트
        rPPG = rPPG * self.output_scale.view(1, 1, 1) + self.output_bias.view(1, 1, 1)

        return rPPG.squeeze(1)  # (B, D)
